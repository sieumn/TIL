...
- 실시간 틱 데이터 수신과 관련된 주의 사항 및 의견을 공유했다.
  + 장 중에 실시간 틱 데이터를 수신하기 위해서는 while Ture 내에서 데이터를 끊임없이 받아오는 동작을 실행 시켜야 한다. 만약, 데이터 수신 외에 매매, 차트 저장, 전략 확인 등 다른 함수 호출이 필요하다면, 해당 함수들 또한 동일한 while문 내에서 호출해야 하는 구조다.
